def sudoku(f):
    ...
    for nl, l in enumerate(f):
        try:
            ...
            P("the line " + str(nl + 1) + " contains something other than a number.")
            return
        if len(n) != 9:
            P("the line " + str(nl + 1) + " does not contain 9 digits.")
            return
        t += [[nl, i] for i in range(9) if n[i] == 0]
        s.append(n)
    if nl != 8:
        P("The sudoku contains " + str(nl + 1) + " lines instead of 9.")
        return

    for l in range(9):
        if ec(s[l]):
            P("the line " + str(l + 1) + " is contradictory.")
            return
    for c in range(9):
